...
public class SchoolPlayer {
	 ...
	 * @return A list of adjacent tiles.
	 */
	private ArrayList<Tile> get_adjacent_tiles(Tile t) {
		ArrayList<Tile> adjacent = new ArrayList<Tile>();

		if (get_box_type(t.x, t.y + 1) != null) {
			adjacent.add(new Tile(t.x, t.y + 1));
		}
		if (get_box_type(t.x, t.y - 1) != null) {
			adjacent.add(new Tile(t.x, t.y - 1));
		}
		if (get_box_type(t.x + 1, t.y) != null) {
			adjacent.add(new Tile(t.x + 1, t.y));
		}
		if (get_box_type(t.x - 1, t.y) != null) {
			adjacent.add(new Tile(t.x - 1, t.y));
	 ...
	 */
	private boolean is_unexplored(Tile t) {
		return get_adjacent_tiles(t).size() != 4 ||
				t.x + offset_x == 0 || t.x + offset_x == map_width-1 ||
				t.y + offset_y == 0 || t.y + offset_y == map_height - 1;
	}
	...
	private ArrayList<Tile> fast_path_to_target(Tile src, Tile dest) {
		...
		while(possible.get(possible.size() - 1).x != dest.x) {
			if(possible.get(possible.size() -1).x < dest.x)
				possible.add(new Tile(possible.get(possible.size()-1).x + 1, 
						possible.get(possible.size()-1).y));
			else
				possible.add(new Tile(possible.get(possible.size()-1).x - 1, 
						possible.get(possible.size()-1).y));
		}
		while(possible.get(possible.size() - 1).y != dest.y) {
			if(possible.get(possible.size() -1).y < dest.y)
				possible.add(new Tile(possible.get(possible.size()-1).x, 
						possible.get(possible.size()-1).y + 1));
			else
				possible.add(new Tile(possible.get(possible.size()-1).x, 
						possible.get(possible.size()-1).y - 1));
		}
	...
	private ArrayList<Tile> get_path_to_best_target(ArrayList<Tile> possible) {
		...
		for (Tile t : possible) {
			if(fast_path_to_target(new Tile(player_x, player_y), t) != null) {
				all_targets.add(fast_path_to_target(new Tile(player_x, player_y), t));
			} else {
